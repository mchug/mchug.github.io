...
function radioClick(stopAudio) {
    ...
    if (audio && currentPlayer && stopAudio) {
        if (!audio.paused) {
            playerPause();
        }
    }
    ...
    if (stream.paused) {
        ...
        $("#navbar-radioIcon").removeClass("ricon-play").addClass("ricon-pause");
        stream.load();
        stream.play();
    }
    else {
        $("#radioIcon").removeClass("ricon-pause").addClass("ricon-play");
        $("#navbar-radioIcon").removeClass("ricon-pause").addClass("ricon-play");
        stream.pause();
    }
}

function getSongTitle() {
    $.get("/php/getSong.php", function (msg) {
        $("#songTitle").text(msg);
    });
}
